...
- `package.json` with `wrangler`, `typescript`, `@cloudflare/workers-types`, and a `types` script that runs `wrangler types`
- `tsconfig.json` with strict mode, ESNext target, and `"types": ["@cloudflare/workers-types/2026-04-01"]` (date matches your `compatibility_date`)
- `src/worker.ts` with a typed `Env` interface and `satisfies ExportedHandler<Env>`
...
The canonical testing path is `@cloudflare/vitest-pool-workers` (current stable v0.13.x, March 2026; requires Vitest 4.1+). It runs tests inside the Workers runtime via Miniflare, with bindings mocked or real (your choice), HMR for fast reruns, and isolated per-test storage.

See [references/testing-with-vitest-pool.md](references/testing-with-vitest-pool.md) for setup details. The pattern:

- `vitest.config.ts` uses `defineWorkersConfig` (or `cloudflareTest()` plugin) referencing your `wrangler.jsonc`
- `tsconfig.json` for tests includes `"@cloudflare/vitest-pool-workers"` in `types`
